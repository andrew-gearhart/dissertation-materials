...
{
	char * amp_name = "amps";
	char * min_amp_name = "min-amps";
	char * max_amp_name = "max-amps";

	if (i < wu_num_fields) {
		if (wu_fields[i].enable) {
			double val = strtod(p->field[i], NULL);
			if (wu_fields[i].name)
				if (strcasecmp(wu_fields[i].name, amp_name) == 0 || strcasecmp(wu_fields[i].name, min_amp_name) == 0 || strcasecmp(wu_fields[i].name, max_amp_name) == 0) { 
 					snprintf(buf, 256, "%.3f", val / 1000.0);
					return 1;
				}
